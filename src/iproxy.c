...
static void print_device(){
	...
	count = usbmuxd_get_device_list(&dev_list);
	if(count <=0){
		printf("get device count error %d\n",count);
	}else{
		printf("get device ok count :%d\n",count);
		strncpy(device_id,dev_list[0].udid,41);
		device_id[41]='\0';
...
typedef struct {
 ...
 static void byteConvert(char * buffer){
...
int main(int argc , char **argv){
...

	{
		...
		c_sock = usbmuxd_connect(dev_list->handle, 2345);
			...
			do{
				...
				char * pinput = gets(buffer);
				//frame.frameSize = strlen(pinput);
				//memcpy(buffer,&frame,16);	
				//byteConvert(buffer);
				//byteConvert(buffer+4);
				//byteConvert(buffer+8);
				//byteConvert(buffer+12);
		
				int len = socket_send(c_sock, pinput, strlen(pinput));
				if(strncmp(buffer,"exit",4) ==0){
					printf("exit ok ");
					return 0;				
				}
				printf("send data len :%d",len);
			}while(1);

			//int len  = socket_send(c_sock, hello, strlen(hello));
